...
void ReadObj::read(const std::string& filename) {
	...
	ifs.open(filename);
	if (!ifs.is_open()) {
		auto& generator = GenerateObj::getInstance();
		generator.generate();
		ifs.open(filename);
	}
	std::string line;
...
void ReadObj::convert(CompleteObject& CObj)const {
	int objOffset = 0;
	...
	for (auto& currentObj : objVector) {
		...
		for (auto& face : currentObj.faceVector) {
			for (auto& currentVertex : face.myData) {
				...
				tempVertex.Pos = XMFLOAT3(currentObj.vertexPos[currentVertex.vertex - 1].x,
										  currentObj.vertexPos[currentVertex.vertex - 1].y,
										  currentObj.vertexPos[currentVertex.vertex - 1].z);

				tempVertex.Tex = XMFLOAT2(currentObj.vertexTex[currentVertex.texture - 1].x,
					currentObj.vertexTex[currentVertex.texture - 1].y);

				tempVertex.Norm = XMFLOAT3(currentObj.vertexNorm[currentVertex.normal - 1].x,
										   currentObj.vertexNorm[currentVertex.normal - 1].y,
									       currentObj.vertexNorm[currentVertex.normal - 1].z);
	...
	CObj.setVectors(vertexVector, indexVector);
}