this.player <- this.inherit("scripts/entity/tactical/human", {
	...
	function onUpdateInjuryLayer()
	{
		...
		else
		{
			...
			else
			{
				injury_body.Visible = true;
				if (this.m.SexH == 1){
					//this.logDebug("I'm a woman named " + this.m.Name);
					return;
				}
				if (this.getSprite("body").getBrush().Name == "bust_hexen_charmed_body_01"){
					//this.logDebug("I'm a trans woman named " + this.m.Name);
					this.m.SexH = 1;
					return;
				}
				//this.logDebug("I'm a man named " + this.m.Name);
	...
	//modded function resets appearance after surviving being struck down
	function deHexe()
	{	
		if (this.m.SexH == 1){
			//this.logDebug("My name is " + this.m.Name + " and I'm about to get dehexed");
			local charmBody = this.getSprite("body");
			local charmHead = this.getSprite("head");
			local charmHair = this.getSprite("hair");
			charmBody.setBrush(this.m.tempBody);
			charmHead.setBrush(this.m.tempHead);
			charmHair.setBrush(this.m.tempHair);
			//this.logDebug("My name is " + this.m.Name + " and I just got dehexed");
		}
	}
		
	function onDeath( _killer, _skill, _tile, _fatalityType )
	{
		//this is the corpse spawn. Handling the player death is in deHexe
///////////////////////////////////
		if (this.m.SexH == 1){
		if (_tile != null)
		{
			this.m.tempBody = this.getSprite("body").getBrush().Name;
			this.m.tempHead = this.getSprite("head").getBrush().Name;
			this.m.tempHair = this.getSprite("hair").getBrush().Name;
			...
			//this.logDebug("My name is " + this.getName() + " and I just died");
		...
		{
		///////////////////////////
		//this is the post battle roster info, so it's unisex
			...
			//The rest of the male corpse spawn
...
////////////////////////////////////////
	...
	function setStartValuesEx( _backgrounds, _addTraits = true )
	{
		...
		//mod
		//this.logDebug("Sex is " + this.m.SexH + " before background assigned");
		this.m.SexH = this.m.Background.getSexC();
		//this.logDebug("Sex is " + this.m.SexH + " after background assigned");
		...
		if (this.m.Name.len() == 0)
		{
			...
			if (this.m.SexH == 0){
				...
				if (background.m.Names[0] == "Sigurd"){
					this.m.Name = this.Const.Strings.CharacterNamesFemale[this.Math.rand(0, this.Const.Strings.CharacterNamesFemale.len() - 1)];
					}
				else if (background.m.Names[0] == "Smud"){
					this.m.Name = this.Const.Strings.CharacterNamesFemaleNorse[this.Math.rand(0, this.Const.Strings.CharacterNamesFemaleNorse.len() - 1)];
				}
				else if (background.m.Names[0] == "Jamil"){
					this.m.Name = this.Const.Strings.SouthernFemaleNames[this.Math.rand(0, this.Const.Strings.SouthernFemaleNames.len() - 1)];
				}
				else if (background.m.Names[0] == "Sir Roderick"){
					this.m.Name = this.Const.Strings.LadyNames[this.Math.rand(0, this.Const.Strings.LadyNames.len() - 1)];
				}
				else {
					this.m.Name = "Nauphy";
				}
		...
		//this.logDebug("Sex is " + this.m.SexH + " at end of ex function");
	}
