this.character_background <- this.inherit("scripts/skills/skill", {
	...
	{	
		if (this.m.SexC == 1){
			this.m.Faces = this.Const.Faces.Female;
			this.m.Hairs = this.Const.Hair.Female;
			this.m.Bodies = this.Const.Bodies.Female;
	...
	function onAdded()
	{
		...
		if (this.m.IsNew && !(("State" in this.Tactical) && this.Tactical.State != null && this.Tactical.State.isScenarioMode()))
		{
			...
			if (actor.getTitle() == "" && this.m.Titles.len() != 0 && this.Math.rand(0, 3) == 3)
			{
				...
				//noblewoman is the only female exclusive title
				else if (this.m.Titles[0] == "Count"){
					actor.setTitle(this.Const.Strings.NoblewomanTitles[this.Math.rand(0, this.Const.Strings.NoblewomanTitles.len() - 1)]);
				}
				else {
					actor.setTitle(this.m.Titles[this.Math.rand(0, this.m.Titles.len() - 1)]);
				}
