...
Lab4_Q3D::Lab4_Q3D() : Scenario() {
	...
	ObjectPresets leftSphere(shapeHelpers::sphere, "obj0.obj", XMMatrixTranslation(-3, 0, sqrt(2.0)), XMVectorSet(3, 0, 0, 0), false, 10);
	leftSphere._initData._sphere._radius = 1.0f;
	_objectPresets.push_back(leftSphere);

	ObjectPresets rightSphere(shapeHelpers::sphere, "obj0.obj", XMMatrixTranslation(3, 0, 0), XMVectorSet(-6, 0, 0, 0), false, 25);
	rightSphere._initData._sphere._radius = 1.0f;
	_objectPresets.push_back(rightSphere);
}

void Lab4_Q3D::generateObjects() const {
